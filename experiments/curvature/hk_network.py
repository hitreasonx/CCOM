...
as the size of the HK network increases.
"""
...
exact_txt_dir = EXACT_CURVATURES_DIR / "hk"
os.makedirs(exact_txt_dir, exist_ok=True)

# Record the errors of each approximation method on HK networks of different scales
results = []
...
for size in sizes:
    print(f"\n========== Processing HK network: n={size} ==========")

    # Generate an HK network
    G = nx.powerlaw_cluster_graph(n=size, m=m, p=p, seed=seed)
    ...
    exact_txt_path = os.path.join(exact_txt_dir, f"hk_{size}.txt")
    exact_curvatures = []
...
plot_error_curve(
    ...
    title="Curvature Approximation Error on HK Networks",
    save_filename="hk_curvature_approximation_mae.png",
...
plot_error_curve(
    ...
    title="Curvature Approximation Error on HK Networks",
    save_filename="hk_curvature_approximation_mre.png",
    legend_loc="upper right",
    legend_bbox=(1.01, 1.016),
    y_as_percent=True
)
