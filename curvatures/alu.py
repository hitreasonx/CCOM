...
class ALU:
    """Approximate Ollivier-Ricci curvature using ALU.

    This class computes an approximate Ollivier-Ricci curvature for each edge
    based on the ALU formula. The computed curvature values are stored as edge
    attributes named ``'alu'``.
    """

    def __init__(self, graph: nx.Graph):
        """Initialize the ALU calculator with a copy of the input graph.
    ...
    def calculate_all_edges_alu(self):
        """Compute ALU-based approximate Ollivier-Ricci curvature for every edge.

        For each edge, the number of common neighbors is counted first, which
        corresponds to the number of 3-cycles containing that edge. The ALU-based
        approximate Ollivier-Ricci curvature is then computed and stored as the
        edge attribute ``'alu'``.

        Returns:
            networkx.Graph: A copy of the input graph with ALU-based approximate
        ...
        # and the corresponding ALU approximate curvature.
        for x, y in self.G.edges:
            num_of_3_cycles = len(neighbors[x] & neighbors[y])
            self.G.edges[x, y]['alu'] = self.calculate_alu(
                self.G.degree[x],
                self.G.degree[y],
                num_of_3_cycles
            )
    ...
    def calculate_alu(self, d_x, d_y, num_of_3_cycles):
        """Compute the ALU approximate curvature for a single edge.
            ...
            Returns:
                float: The ALU approximate curvature value of the edge.
        """
        ...
        result = ((part1 + part2 + part3) + part3) / 2
        return result
